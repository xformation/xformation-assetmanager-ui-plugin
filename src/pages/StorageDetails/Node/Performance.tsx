...
export class Performance extends React.Component<any, any> {
  ...
  constructor(props: any) {
    ...
    this.steps = [
      ...
      },
      {
        ...
        component: () => (
          <EnableDashboard
            ref={this.enableDashboardRef}
            inputName={this.state.inputName}
            selectedData={
              this.verifyInputsRef.current !== null
                ? this.verifyInputsRef.current.getSelection()
                : null
            }
          />
        ),
      },
      {
        ...
        component: () => (
          <Preview
            ...
            selectedDashboards={
              this.enableDashboardRef.current !== null
                ? this.enableDashboardRef.current.getSelection()
                : null
            }
          />
        ),
  ...
  getViewJson = async () => {
    try {
      ...
      const tenantId = this.getParameterByName(
        "tenantId",
      ...
      await fetch(
        `${config.GET_VIEW_JSON}?cloudType=${cloud}&elementType=${type}&inputType=${this.state.inputName}&accountId=${accountId}&tenantId=${tenantId}`,
        reqOptions
      ).then(
        (response: any) => {
          if (response) {
  ...
  getInputConfig = async () => {
    try {
      ...
      console.log(
        "Performance. Excepiton in search input config. Error: ",
        err
      );
    }
  };
  ...
  updateDashboardsStatusInAppAsset() {
    ...
    RestService.add(
      `${config.BULK_UPDATE_APPLICATION_ASSETS}`,
      updatedDashboards
    )
      .then((response: any) => {
  ...
  render() {
    ...
    if (
      viewJson &&
      viewJson.dashboards &&
      viewJson.dashboards[activeDashboard]
    ) {
          ...
          <>
            <div className="performance-box">
              <div className="performance-inner">
                ...
                <p>
                  To endble Performance Monitoring dashboards you will first
                  have to configure the inputs for data collection
                </p>
                <button
                  className="asset-blue-button"
                  onClick={this.enablePerformanceMonitoring}
                >
                    ...
                    {activeDB && (
                      <>
                        <iframe
                          ...
                          src={`/justdashboard?uid=${activeDB.Uid}&slud=${activeDB.Slug}`}
                          onLoad={() => {
